...
## Mục đích
File `CLAUDE.md` trong repository này định nghĩa một generic AI Agent Skill dùng cho mọi agent, với định hướng architecture-first và quy trình 7 bước.

## Nội dung chính
- Persona và rules chung cho agent.
- Workflow 7 bước: `/spec`, `/plan`, `/arch`, `/cook`, `/review`, `/fix`, `/final`.
- Metadata `scope` và `applyTo` để hỗ trợ áp dụng cho nhiều loại agent.
- Hướng dẫn sử dụng trong môi trường VS Code.

## Nội dung bổ sung
- `SKILL.md` cung cấp tóm tắt về cách dùng skill này làm baseline cho mọi agent.
- `USE_GUIDE.md` hướng dẫn cách tích hợp bộ skill chung vào từng dự án cụ thể.

## Hướng dẫn nhanh
1. Đọc `CLAUDE.md` để hiểu cách agent nên hành xử.
2. Tham chiếu `.claudecode/workflow/*` khi cần chi tiết hơn.
3. Sử dụng `SKILL.md` như tài liệu chung nếu bạn muốn áp dụng skill này cho toàn bộ workspace.
4. Đọc `USE_GUIDE.md` để biết cách tạo và duy trì file tổng quan dự án riêng.
5. Nếu cần cải tiến thêm, cập nhật `CLAUDE.md` và metadata `scope`/`applyTo` để mở rộng phạm vi.
...
Tech Stack
Frontend: Vite + React 18 + TypeScript + Tailwind CSS + Framer Motion Backend: Supabase (PostgreSQL + Realtime) Deployment: Vercel i18n: react-i18next Forms: React Hook Form + Zod State: React Context + TanStack Query
...
./docs/project-overview-pdr.md — Project vision, goals, feature scope
./docs/code-standards.md — Coding conventions, folder structure, TypeScript rules
./docs/design-system/design-principles.md — Design system, colors, typography, motion
./docs/project-roadmap.md — 8-phase implementation plan (9 weeks)
./docs/system-architecture.md — Technical architecture, data flow, DB schema
./docs/deployment-guide.md — Vercel & Supabase setup, deployment steps
Development
Commands
...
Vercel Setup
Push code to GitHub
Go to vercel.com, create new project
...
Run database schema setup (see ./docs/deployment-guide.md)
Configure RLS policies
Populate guests table
Copy Project URL and anon key to Vercel env vars
See ./docs/deployment-guide.md for detailed setup instructions.

Roadmap
Phase 0: Setup (Vite, React, TypeScript, Tailwind, Supabase) ✓ Complete Phase 1: Hero + Countdown + i18n + Dark mode ✓ Complete Phase 2: Our Story + Event Details + Add-to-calendar ✓ Complete Phase 3: Album + Video + Cinematic backdrop ✓ Complete Phase 4: RSVP + Personalized link + Guests table (In Planning) Phase 5: Wishes + QR/Bank gift Phase 6: Mini-games + Leaderboard Phase 7: OG image + Share buttons + Floating UI Phase 8: Polish + Accessibility + Performance + Deploy
...
Commit with conventional commits: git commit -m "feat: add hero section"
Push to GitHub: git push origin feat/your-feature
Create pull request
Ensure all checks pass (tests, linting, Lighthouse)
Support
Documentation: ./docs/
